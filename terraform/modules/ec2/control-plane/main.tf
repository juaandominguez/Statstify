resource "aws_instance" "kubernetes-control-plane" {
  ...
  user_data            = templatefile("${path.module}/startup-control-plane.tpl.sh", { S3_BUCKET_NAME = var.s3_bucket_name, SQS_QUEUE_NAME = var.sqs_queue_name, AWS_REGION = var.region })
